...
<?php
    ...
    $total_e_pop_a1_prov =\DB::connection('mysql')->select("call vaccine_data_province($province_id,'','')")[0]->a1; //Frontline(A1) SINOVAC_FIRST
    $total_e_pop_a2_prov  = \DB::connection('mysql')->select("call vaccine_data_province($province_id,'','')")[0]->a2; //Senior (A2) SINOVAC_FIRST
    $total_e_pop_svac_prov = $total_e_pop_a1_prov + $total_e_pop_a2_prov; //TOTAL ELI POP SINOVAC_FIRST


    //ELIGIBLE_POP_ASTRAZENECA
    $total_e_pop_astra_frtline_prov = \DB::connection('mysql')->select("call vaccine_data_province($province_id,'','')")[0]->a1; //Frontline(A1) ASTRA_FIRST
    $total_e_pop_astra_sr_prov = \DB::connection('mysql')->select("call vaccine_data_province($province_id,'','')")[0]->a2; //SENIOR(A2) ASTRA_FIRST
    ...
    $total_vcted_svac_frst_prov = \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->vaccinated_first; //TOTAL VACCINATED SINOVAC_FIRST
    $total_vcted_astra_frst_prov = \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->vaccinated_first; //TOTAL VACCINATED ASTRA_FIRST

    $total_vcted_svac_scnd_prov = \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->vaccinated_second; //TOTAL VACCINATED SINOVAC_SECOND
    $total_vcted_astra_scnd_prov = \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->vaccinated_second; //TOTAL VACCINATED ASTRA_SECOND

    $total_mild_svac_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->mild_first; //MILD SINOVAC_FIRST
    $total_mild_astra_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->mild_first; //MILD ASTRA_FIRST

    $total_mild_svac_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->mild_second; //MILD SINOVAC_SECOND
    $total_mild_astra_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->mild_second; //MILD ASTRA_SECOND

    $total_srs_svac_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->serious_first; //SERIOUS SINOVAC_FIRST
    $total_srs_astra_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->serious_first; //SERIOUS ASTRA_FIRST

    $total_srs_svac_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->serious_second; //SERIOUS  SINOVAC_SECOND
    $total_srs_astra_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->serious_second; //SERIOUS ASTRA_SECOND

    $total_dfrd_svac_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->deferred_first; //DEFERRED SINOVAC_FIRST
    $total_dfrd_astra_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->deferred_first; //DEFERRED ASTRA_FIRS

    $total_dfrd_svac_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->deferred_second; //DEFERRED  SINOVAC_SECOND
    $total_dfrd_astra_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->deferred_second; //DEFERRED ASTRA_SECOND

    $total_rfsd_svac_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->refused_first; //REFUSED SINOVAC_FIRST
    $total_rfsd_astra_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->refused_first; //REFUSED ASTRA_FIRST

    $total_rfsd_svac_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->refused_second; //REFUSED  SINOVAC_SECOND
    $total_rfsd_astra_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->refused_second; //REFUSED ASTRA_SECOND

    $total_wstge_svac_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->wastage_first; //WASTAGE SINOVAC_FIRST
    $total_wstge_astra_frst_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->wastage_first; //WASTAGE ASTRA_FIRST

    $total_wstge_svac_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Sinovac','')")[0]->wastage_second; //WASTAGE SINOVAC_SECOND
    $total_wstge_astra_scnd_prov =  \DB::connection('mysql')->select("call vaccine_data_province($province_id,'Astrazeneca','')")[0]->wastage_second; //WASTAGE ASTRA_SECOND
...
<h4>Grand Total</h4>
    ...
    <tbody id="collapse_sinovac_grand" class="collapse bg-danger" aria-labelledby="headingTwo" data-parent="#accordionExample">
        <tr style="background-color: #ffd8d6">
            ...
            <td rowspan="2">{{ $total_e_pop_a1_prov }}</td>  <!-- Frontline(A1) SINOVAC_FIRST-->
            <td rowspan="2">{{ $total_e_pop_a2_prov }} </td>   <!-- Senior (A2) SINOVAC_FIRST -->
            <td rowspan="2"></td>
            <td rowspan="2"></td>
