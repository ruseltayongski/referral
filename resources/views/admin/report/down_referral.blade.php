            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="box-header with-border">
                        <h3>Down Referral</h3>
                        <form action="{{ asset('admin/down/referral') }}" style="margin-top:20px;display:flex;" id="myForm" method="POST" class="form-inline">
                            {{ csrf_field() }}
                ...
                <table class="table table-bordered table-striped table-hover table-fixed-header" style="border: ">
                    ...
                    <tbody>
                        @foreach($data as $key => $row)
                            @if (
                                $row['coordinate'] != 'RHU TO RHU' &&
                                $row['coordinate'] != 'RHU TO LEVEL 1' &&
                                $row['coordinate'] != 'RHU TO LEVEL 2' &&
                                $row['coordinate'] != 'RHU TO LEVEL 3' &&
                                $row['coordinate'] != 'LEVEL 1 TO LEVEL 1' &&
                                $row['coordinate'] != 'LEVEL 1 TO LEVEL 2' &&
                                $row['coordinate'] != 'LEVEL 1 TO LEVEL 3' &&
                                $row['coordinate'] != 'LEVEL 2 TO LEVEL 2' &&
                                $row['coordinate'] != 'LEVEL 2 TO LEVEL 3' &&
                                $row['coordinate'] != 'LEVEL 3 TO LEVEL 3'
                            )
                                @if (isset($row['total']) && $key != count($data) - 1)
                                    @if (!$firstTotalSkipped)
                                        @php $firstTotalSkipped = true; @endphp
